...
updateMenu.addAction("Update Employee", true, async () => {
    ...
    while (!hasChosenEmployee) {
        await inquirer.prompt([
            ...
            if (!which_employee.trim()) {
                console.log("Please type a name or ID");

            } else if(which_employee.match(/^\d+$/g)) {
...
reportsMenu.addAction("View all employees", true, async () => {
    const results = await queries.selectAll("employee");

    return console.table(results);
})

reportsMenu.addAction("View all departments", true, async () => {
    const results = await queries.selectAllInnerJoin("employee", "employee manager", "manager_id", "id" ,["employee.*", "manager.first_name"]);
