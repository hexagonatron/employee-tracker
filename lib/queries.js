...
module.exports = {
    ...
    selectAllInnerJoin: async (tableOneName, tableTwoName, tableOneForeignKeyName, tableTwoPrimaryKey, cols) => {
        if(tableTwoName.split(' ').length > 1){
            const tableTwoAlias = tableTwoName.split(' ')[1];
            tableTwoName = tableTwoName.split(' ')[0];
        
        }
        const results = await connection.queryAsync("SELECT ?? FROM ?? INNER JOIN ?? ?? ON ??.??=??.??", [cols, tableOneName, tableTwoName, tableTwoAlias|| tableTwoName, tableOneName, tableOneForeignKeyName, tableTwoName, tableTwoPrimaryKey]);
