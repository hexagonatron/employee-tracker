...
module.exports = {
    ...
    selectAllInnerJoin: async (tableOneName, tableTwoName, tableOneForeignKeyName, tableTwoPrimaryKey, cols) => {
        ...
        const results = await connection.queryAsync("SELECT ?? FROM ?? INNER JOIN ?? ?? ON ??.??=??.??", [cols, tableOneName, tableTwoName, tableTwoAlias?tableTwoAlias:tableTwoName, tableOneName, tableOneForeignKeyName, tableTwoName, tableTwoPrimaryKey]);
